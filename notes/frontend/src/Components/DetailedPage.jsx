...
const DetailedPage = () => {
  ...
  useEffect(() => {
    const fetchTopicData = async () => {
      try {
        // Fetch all topics
        const topicsResponse = await httpClient.get("/topics");
        setTopics(topicsResponse.data);

        // Find the selected topic by ID
        const selectedTopic = topicsResponse.data.find(
          (topic) => topic._id === id
        );
        ...
        if (selectedTopic) {
          ...
          const subtopicsResponse = await axios.get(
            `http://localhost:4001/topics/${id}/subtopics`
          );
  ...
  const handleSubtopicClick = async (topicId, subtopicId) => {
    console.log(
      `Fetching PDF for Topic ID: ${topicId}, Subtopic ID: ${subtopicId}`
    );
    try {
      // Fetch PDF URL for the selected subtopic
      const response = await axios.get(
        `http://localhost:4001/files/${topicId}/${subtopicId}`
      );
  ...
  const handleTopicClick = async (topic) => {
    ...
    if (topic) {
      try {
        // Fetch subtopics of the selected topic
        const response = await axios.get(
          `http://localhost:4001/topics/${topic._id}/subtopics`
        );
