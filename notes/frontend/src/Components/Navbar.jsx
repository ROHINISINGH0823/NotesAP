  ...
  const handleSearchSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:4001/search?tag=${searchQuery}`
      );
    ...
    <>
      <div
        className={`max-w-screen-2xl container mx-auto md:px-20 px-4 ${
          sticky
            ? "sticky-navbar b-blue-500 text-white"
            : "normal-navbar b-black text-white"
        } fixed top-0 left-0 right-0 z-50 duration-300 transition-all ease-in-out`}
      >
        <div className="navbar nav">
          ...
          <div className="navbar-end space-x-3">
            ...
            {authUser ? (
              <button onClick={handleLogout} className="btn #F4ECF7">
                Logout
              </button>
            ) : (
