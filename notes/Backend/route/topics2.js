...
router.get('/', async (req, res) => {
  try {
    const topics = await Topic2.find();
    res.json(topics);
  } catch (err) {
...
router.get('/:id/subtopics', async (req, res) => {
  try {
    ...
    const topic = await Topic2.findById(id);
    if (!topic) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const newTopic2 = new Topic2({ name, subtopics, pdfPath });

    // Save the new topic to the database
    const savedTopic2 = await newTopic2.save();

    // Return the saved topic as JSON response
    res.status(201).json(savedTopic2);
  } catch (err) {
